...
<body>
	<div class="row">
		...
		<div class="col-sm-8">
			<?php
				...
				"
					<div>
						<div><img id='cover-img' class='img-rounded' src='cover/$user_cover' alt='cover'></img></div>
						<form action ='profile.php?u_id=$user_id' method='post' enctype='multipart/form-data'> 
							<ul class='nav pull-left' style='position:absolute; top:10px; left:40px;'>
								<li class='dropdown'>
									<button class='dropdown-togggle btn btn-default' data-toggle='dropdown'>Change Cover</button>
									<div class='dropdown-menu'>
										<center>
											<p>Click <strong>Select Cover</strong> and then click the <br> <strong>Update Cover</strong></p>
											<label class='btn btn-info'>Select Cover<input type='file' name='u_cover' size='60'></input>
											</label><br><br>
					...
					<div id='profile-img'>
						<img src='users/$user_image' alt='Profile' class='img-circle' width = '180px' height='185px'></img>
						<form action='profile.php?u_id=$user_id' method='post' enctype='multipart/form-data'>
	...
	<div class="row">
		...
		<div class="col-sm-2" style="background-color: #B9FFDD; text-align: center;left: 0.9%; border-radius: 5px;">
			<?php
				echo
				"
		...
		<div class="col-sm-6">
			<?php
				...
				while ($row_posts = mysqli_fetch_array($run_posts)) {
					...
					/*else if (strlen($content) >= 1  && strlen($pet_type) >= 1 && strlen($pet_name) >= 1 && strlen($pet_age) >= 1 && strlen($pet_gender) >= 1 && strlen($pet_location) >= 1) {
						echo
						"	
						<div id = 'own_posts'>
							<div class='row'>
						...
						$user = $_SESSION['user_email'];
						$get_posts = "SELECT * FROM users WHERE user_email='$user'";
						$run_user = mysqli_query($con, $get_user);
						$row = mysqli_fetch_array($run_user);
